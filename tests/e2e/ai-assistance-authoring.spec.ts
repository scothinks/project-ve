...
test('quiz assistance preserves manual drafts, selects questions and recovers a lost save response', async ({ browser, baseURL }, info) => {
  ...
  try {
    ...
    await drawer.getByRole('button', { name: 'Check cost', exact: true }).click();
    await drawer.getByRole('button', { name: 'Generate questions', exact: true }).click();
...
test('lesson suggestions quote drafting separately, retain refinements and add a reviewable draft', async ({ browser, baseURL }, info) => {
  ...
  try {
    ...
    let drawer = page.getByRole('dialog');
    await expect(drawer.getByText('Suggestions only. Drafting is quoted separately.')).toBeVisible();
    await drawer.getByRole('button', { name: 'Suggest lessons', exact: true }).click();
    ...
    await drawer.getByRole('button', { name: 'Draft lesson', exact: true }).click();
