...
test("page generation retains candidates, recovers navigation and saves once", async ({ browser, baseURL }, info) => {
  ...
  try {
    ...
    await expect(drawer.getByRole("button", { name: "Recommend · 0 credits", exact: true })).toBeEnabled({ timeout: 30_000 });
...
test("ready results recover a lost save response and retain failed refinements", async ({ browser, baseURL }) => {
  ...
  try {
    ...
    await expect(drawer.getByRole("button", { name: "Create · 0 credits", exact: true })).toBeEnabled();
    await drawer.getByLabel("What would you like to change?").fill("Use a school example");
    await drawer.getByRole("button", { name: "Update price" }).click();
    await drawer.getByRole("button", { name: "Create · 0 credits", exact: true }).click();
