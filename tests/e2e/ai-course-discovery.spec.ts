...
test('uncertain authors shape an idea and learners; complete briefs take the direct path', async ({ browser, baseURL }, info) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: 'Generate · 0 credits', exact: true })).toBeEnabled();
...
test('reopened expired requests refresh without starting and can return to an editable brief', async ({ browser, baseURL }) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: 'Generate · 0 credits', exact: true })).toBeDisabled();
...
test('metered price changes require a new click; double clicks and a lost Start keep one intent', async ({ browser, baseURL }) => {
  ...
  try {
    ...
    const generate = page.getByRole('button', { name: 'Generate · 57 credits', exact: true });
    await expect(generate).toBeEnabled();
    await expect(page.getByText(/205 credits. Total: 262 credits/)).toBeVisible();
    ...
    await page.getByRole('button', { name: 'Generate · 57 credits', exact: true }).click();
