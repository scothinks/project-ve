    ...
    <div className="-mx-5 md:-mx-8">
      ...
      <div className="flex justify-center px-6 py-12 md:py-24">
        <div className="flex w-full max-w-[640px] flex-col gap-6">
          <span
            className="inline-flex w-fit items-center gap-1.5 rounded-full px-3.5 py-[5px] text-[11px] font-extrabold uppercase tracking-[0.08em] text-white"
            style={{ background: "linear-gradient(135deg, var(--admin-accent-violet, #8d68f2), var(--admin-accent-sky, #6fa8ff))" }}
          >
          ...
          <div>
            <h1 className="text-[32px] font-black leading-[1.15] tracking-[-0.01em] text-[var(--admin-brand-hero)]">
              {option.title}
            </h1>
            <p className="mt-2.5 text-[15px] font-medium leading-[1.6] text-[var(--admin-on-surface-variant)]">
              {option.description}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            {option.lessonOutline.map((lesson, index) => (
              <div
                className="flex items-center gap-3.5 rounded-[16px] border border-dashed border-[var(--admin-border-warm)] px-4 py-3.5"
                key={lesson.title}
              >
                <span className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[9px] bg-[var(--admin-surface-container-low)] text-xs font-extrabold text-[var(--admin-on-surface-variant)]">
                  {index + 1}
                </span>
                <span className="text-sm font-bold text-[var(--admin-on-surface)]">{lesson.title}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-2 flex gap-2.5">
            <form action={useAiBriefOutline} className="flex-1">
              ...
              <PendingSubmitButton
                className="flex w-full items-center justify-center gap-2 rounded-full bg-[var(--admin-primary)] px-5 py-[15px] text-sm font-extrabold text-[var(--admin-on-primary)] disabled:opacity-60"
            ...
            <Link
              className="inline-flex items-center justify-center rounded-full border border-[var(--admin-border-warm)] px-[22px] py-[15px] text-sm font-bold text-[var(--admin-on-surface)]"
              href="/admin/courses/ai/brief"
            >
              Try again
            </Link>
