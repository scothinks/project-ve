    ...
    <div className="-mx-5 md:-mx-8">
      ...
      <div className="flex justify-center px-6 py-12 md:py-24">
        <div className="flex w-full max-w-[600px] flex-col gap-8">
          <div className="flex flex-col items-center gap-3.5 rounded-[22px] bg-[var(--ui-surface-soft)] px-6 py-9 text-center">
            <span
              className="flex h-12 w-12 items-center justify-center rounded-full"
              style={{
                background: ready
                  ? "color-mix(in srgb, var(--ui-action) 14%, transparent)"
                  : "color-mix(in srgb, var(--ui-success) 14%, transparent)",
              }}
            >
              {ready ? (
                <svg aria-hidden="true" className="h-[26px] w-[26px]" fill="none" stroke="var(--ui-action)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.4" viewBox="0 0 24 24">
                  <path d="M20 6 9 17l-5-5" />
                </svg>
              ) : (
                <svg aria-hidden="true" className="h-[26px] w-[26px]" fill="none" stroke="var(--ui-warning)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.4" viewBox="0 0 24 24">
                  <circle cx="12" cy="12" r="9" />
                  <path d="M12 8v5M12 16h.01" />
                </svg>
              )}
            </span>
            <h1 className="text-2xl font-black text-[var(--ui-text)]">
          ...
          <div className="flex flex-col gap-2.5">
            {readiness.checks.map((check) => {
              ...
              const row = (
                <div
                  className="flex items-center gap-3.5 rounded-[16px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface)] px-4 py-3.5"
                  key={check.id}
                >
                  <span
                    ...
                    style={{
                      background: passed
                        ? "color-mix(in srgb, var(--ui-action) 14%, transparent)"
                        : "color-mix(in srgb, var(--ui-warning) 14%, transparent)",
                    }}
                  >
                    {passed ? (
                      <svg aria-hidden="true" className="h-3.5 w-3.5" fill="none" stroke="var(--ui-action)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" viewBox="0 0 24 24">
                        <path d="M20 6 9 17l-5-5" />
                      </svg>
                    ) : (
                      <svg aria-hidden="true" className="h-3.5 w-3.5" fill="none" stroke="var(--ui-warning)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" viewBox="0 0 24 24">
                        <path d="M18 6 6 18M6 6l12 12" />
                      </svg>
                    )}
                  </span>
                  <span className="flex-1 text-sm font-bold text-[var(--ui-text)]">{check.label}</span>
                </div>
              );
          ...
          {!ready && <form action={approveCourseReview} className="space-y-4">
            ...
            <PendingSubmitButton className="rounded-full bg-[var(--ui-action)] px-5 py-3 text-sm font-bold text-[var(--ui-on-action)] disabled:opacity-50" disabled={!readiness.canApprove} label="Approve reviewed course" pendingLabel="Approving…" type="submit"/>
          </form>}
          <Link
          ...
          {ready ? (
            <form action={publishReviewedCourse}>{revisionFields}
              ...
              <PendingSubmitButton
                className="flex w-full items-center justify-center rounded-full bg-[var(--ui-action)] px-6 py-[15px] text-sm font-extrabold text-[var(--ui-on-action)]"
            ...
            <div className="flex flex-col gap-2.5">
              <div className="flex w-full items-center justify-center rounded-full bg-[color:color-mix(in_srgb,var(--ui-action)_35%,var(--ui-surface-soft))] px-6 py-[15px] text-sm font-extrabold text-[var(--ui-on-action)] opacity-60">
                Publish course
              </div>
          ...
          <div className="flex items-center justify-center gap-4 border-t border-[var(--ui-border-subtle)] pt-2">
            <span className="text-xs font-bold text-[var(--ui-text-muted)]">Currently {statusLabel}</span>
            {course.status === "published" ? (
              <form action={unpublishReviewedCourse}>
                <input name="courseId" type="hidden" value={course.id} />
                <button className="text-xs font-extrabold text-[var(--ui-text-muted)] underline" type="submit">
                  Unpublish
                </button>
              </form>
            ) : null}
            {course.status !== "archived" ? (
              <form action={archiveReviewedCourse}>
                <input name="courseId" type="hidden" value={course.id} />
                <button className="text-xs font-extrabold text-[var(--ui-warning)] underline" type="submit">
                  Archive course
                </button>
              </form>
            ) : null}
            {course.status === "archived" ? (
              <form action={restoreCourseToDraft}>
                <input name="courseId" type="hidden" value={course.id} />
                <button className="text-xs font-extrabold text-[var(--ui-action)] underline" type="submit">
                  Restore to draft
                </button>
              </form>
            ) : null}
