...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === "claim") {
      ...
      if (!existing?.completed.length) return NextResponse.json({ savedXp: 0 });
      ...
      if (error) return NextResponse.json({ error: "Your progress could not be saved. Please try again." }, { status: 503 });
      const response = NextResponse.json(data);
      response.cookies.set(receiptCookie, "", { ...options, maxAge: 0 });
      return response;
    }
    ...
    if (action === "learn") {
      ...
      response.cookies.set(receiptCookie, encodeReceipt(receipt, secret), options); return response;
    }
