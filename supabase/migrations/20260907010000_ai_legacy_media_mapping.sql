begin;
-- Retain the complete legacy source, even when a destination cannot be resolved.
create table private.ai_legacy_media_mappings (
 asset_id uuid primary key references public.learning_media_assets(id) on delete cascade,
 course_id text not null references public.courses(id) on delete cascade,
 original jsonb not null,
 target_kind text,
 target_id text,
 status text not null check(status in ('mapped','needs_resolution','deferred')),
 reason text,
 mapped_at timestamptz,
 mapped_by uuid,
 updated_at timestamptz not null default now()
);
alter table private.ai_legacy_media_mappings enable row level security;
revoke all on private.ai_legacy_media_mappings from public,anon,authenticated,service_role;

create function private.map_legacy_ai_media(p_asset uuid,p_kind text default null,p_target text default null) returns void
language plpgsql security definer set search_path=public,private as $$
declare a public.learning_media_assets; k text; tid text; lid text; existing jsonb; bid text; n integer; block_kind text; intent jsonb; reason text;
begin
 ...
 select b.payload,p.lesson_id into existing,lid from lesson_content_blocks b join lesson_pages p on p.id=b.page_id join lessons l on l.id=p.lesson_id where b.id::text=tid and b.block_type=block_kind and l.course_id=a.course_id and (a.lesson_id is null or a.lesson_id=l.id) for update of b;
 ...
 update private.ai_legacy_media_mappings set status='needs_resolution',reason=left(reason,500),updated_at=now() where asset_id=a.id;
 end;
end $$;
revoke all on function private.map_legacy_ai_media(uuid,text,text) from public,anon,authenticated,service_role;

create function public.admin_map_legacy_ai_media(p_course text,p_asset uuid default null,p_kind text default null,p_target text default null) returns void
language plpgsql security definer set search_path=public,private as $$
declare aid uuid;
begin
 ...
 if p_asset is not null and not exists(select 1 from learning_media_assets where id=p_asset and course_id=p_course) then raise exception 'Brief unavailable.' using errcode='42501'; end if;
 for aid in select id from learning_media_assets where course_id=p_course and (p_asset is null or id=p_asset) order by id loop perform private.map_legacy_ai_media(aid,p_kind,p_target); end loop;
end $$;
revoke all on function public.admin_map_legacy_ai_media(text,uuid,text,text) from public,anon,authenticated,service_role;
grant execute on function public.admin_map_legacy_ai_media(text,uuid,text,text) to authenticated;
...
('public','admin_map_legacy_ai_media','p_course text, p_asset uuid, p_kind text, p_target text','ADMIN_AUTHENTICATED','Course editors','Current course edit access; exact same-course placement; idempotent mapping preserves original rows and approvals.',array['authenticated']);
-- Data backfill runs in the final Phase 5 migration after helper corrections.
