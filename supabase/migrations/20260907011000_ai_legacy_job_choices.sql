begin;
create table private.ai_legacy_job_choices (
 job_id uuid primary key references public.ai_generation_jobs(id) on delete cascade,
 decision text not null default 'pending' check(decision in ('pending','continue','cancel')),
 decided_by uuid,
 decided_at timestamptz,
 created_at timestamptz not null default now()
);
alter table private.ai_legacy_job_choices enable row level security;
revoke all on private.ai_legacy_job_choices from public,anon,authenticated,service_role;
-- Only unclaimed queued requests pause. Running work and its retries keep their leases.
insert into private.ai_legacy_job_choices(job_id) select id from ai_generation_jobs
where job_type='media_assets' and status='queued' and coalesce(prompt->>'mode','')<>'authoring_image_v4';
...
create function public.admin_decide_legacy_ai_job(p_job uuid,p_decision text) returns void
...
create function public.admin_legacy_ai_media_workspace(p_course text) returns jsonb
...
('public','admin_decide_legacy_ai_job','p_job uuid, p_decision text','ADMIN_AUTHENTICATED','Course editors','Current course editor; locked pending choice and original reservation; no new provider dispatch or refund of attempted work.',array['authenticated']),
('public','admin_legacy_ai_media_workspace','p_course text','ADMIN_AUTHENTICATED','Course editors','Current course editor; read-only bounded course workspace; no worker secrets or private job prompts.',array['authenticated']);
