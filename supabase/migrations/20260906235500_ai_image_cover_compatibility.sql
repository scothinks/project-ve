...
begin
 ...
 -- Keep applicable legacy cover requirements attached to their existing identities.
 if target in ('course_thumbnail','lesson_thumbnail') then
 update learning_media_assets a set url='/api/media/'||v.id,alt_text=v.alt_text,source='library',review_status='draft',generation_status='completed',metadata=a.metadata||jsonb_build_object('libraryVersionId',v.id)
 where a.course_id=r.course_id and ((target='course_thumbnail' and a.lesson_id is null and (a.metadata->>'targetKind'='course_thumbnail' or a.asset_type='thumbnail' or lower(a.placement)='course_thumbnail'))
 or (target='lesson_thumbnail' and a.lesson_id=r.lesson_id and (a.metadata->>'targetKind' in ('lesson_cover','lesson_thumbnail') or a.asset_type in ('cover','thumbnail'))));
 end if;
