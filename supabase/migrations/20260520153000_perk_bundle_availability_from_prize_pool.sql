...
as $$
  ...
  where rewards.distribution_mode <> 'perk_bundle'
    and rewards.fulfillment_type in ('voucher_code', 'qr_code')
  ...
  where rewards.distribution_mode <> 'perk_bundle'
    and rewards.fulfillment_type not in ('voucher_code', 'qr_code')
    and rewards.status = 'published'
    and rewards.is_enabled
  group by rewards.id
  ...
  select
    rewards.id as reward_id,
    coalesce(prize_summary.total_prizes, 0)
      + case when prize_summary.fallback_configured then 1 else 0 end as total_uploaded,
    case
      when coalesce(prize_summary.live_prize_count, 0) > 0 or prize_summary.fallback_configured then 1
      else 0
    end as total_available
  from public.rewards
  left join lateral (
    select
      count(*)::integer as total_prizes,
      count(*) filter (
        where p.is_enabled
          and (p.available_from is null or p.available_from <= now())
          and (p.expires_at is null or p.expires_at > now())
          and public.perk_prize_release_bucket_allows(p.id, now())
          and (
            p.total_win_cap is null
            or (
              select count(*)
              from public.perk_bundle_draws draw_total
              where draw_total.prize_id = p.id
                and draw_total.award_status <> 'refunded'
            ) < p.total_win_cap
          )
          and (
            p.daily_win_cap is null
            or (
              select count(*)
              from public.perk_bundle_draws draw_today
              where draw_today.prize_id = p.id
                and draw_today.award_status <> 'refunded'
                and draw_today.created_at >= ((now() at time zone 'Africa/Lagos')::date at time zone 'Africa/Lagos')
            ) < p.daily_win_cap
          )
          and (
            p.prize_type in ('native_xp', 'xp_boost')
            or (
              p.prize_type = 'reward'
              and exists (
                select 1
                from public.rewards source_reward
                where source_reward.id = p.source_reward_id
                  and source_reward.distribution_mode <> 'perk_bundle'
                  and source_reward.status = 'published'
                  and source_reward.is_enabled
                  and (source_reward.starts_at is null or source_reward.starts_at <= now())
                  and (source_reward.ends_at is null or source_reward.ends_at > now())
                  and public.campaign_is_live(source_reward.campaign_id)
                  and (
                    (
                      source_reward.fulfillment_type in ('voucher_code', 'qr_code')
                      and exists (
                        select 1
                        from public.reward_inventory_items assigned_item
                        where assigned_item.reward_id = source_reward.id
                          and assigned_item.perk_prize_id = p.id
                          and assigned_item.status = 'available'
                          and (assigned_item.available_from is null or assigned_item.available_from <= now())
                          and (assigned_item.expires_at is null or assigned_item.expires_at > now())
                          and public.campaign_is_live(assigned_item.campaign_id)
                      )
                    )
                    or (
                      source_reward.fulfillment_type not in ('voucher_code', 'qr_code')
                      and exists (
                        select 1
                        from public.reward_quantity_allocations assigned_quantity
                        where assigned_quantity.reward_id = source_reward.id
                          and assigned_quantity.perk_prize_id = p.id
                          and assigned_quantity.quantity_available > 0
                          and (assigned_quantity.available_from is null or assigned_quantity.available_from <= now())
                          and (assigned_quantity.expires_at is null or assigned_quantity.expires_at > now())
                          and public.campaign_is_live(assigned_quantity.campaign_id)
                      )
                    )
                  )
              )
      ...
      )::integer as live_prize_count,
      (
        jsonb_typeof(rewards.fulfillment_config -> 'fallback') = 'object'
      ) as fallback_configured
    from public.perk_bundle_prizes p
    where p.bundle_reward_id = rewards.id
  ) prize_summary on true
  where rewards.distribution_mode = 'perk_bundle'
    and rewards.status = 'published'
    and rewards.is_enabled;
$$;
