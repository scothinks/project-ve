begin;
create or replace function public.admin_media_library(p_organization_id uuid default null,p_source text default 'organization',p_search text default '',p_type text default 'image',p_offset integer default 0,p_manage boolean default false,p_course_id text default null)
...
begin
 ...
 if p_source not in ('organization','platform','generated','unused') or p_offset < 0 or p_offset > 100000 then raise exception 'Invalid library request.' using errcode='22023'; end if;
 ...
 where (p_source not in ('generated','unused') or v.authoring_result_id is not null)
 and (p_source<>'unused' or not exists(select 1 from private.media_placements mp where mp.version_id=v.id))
 and (p_course_id is null or exists(select 1 from private.media_placements mp where mp.version_id=v.id and mp.course_id=p_course_id))
