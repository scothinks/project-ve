...
Date: 2026-09-10. Planning only; no implementation has begun.
...
not reaching anyone, judge a prize pool by real odds rather than raw weights, and add or move
...
| The perk prize table is ten columns and exposes raw weights. | `app/admin/rewards/perks/[id]/page.tsx`, columns `Prize`…`Weight`…`Remaining total` | Present weight as a share of total draws. Keep `dailyWinCap`, per-prize enable/disable and the draw log intact. |
...
| --- | --- | --- |
| Identity token cutover (`--ui-*`) | Implemented and locally qualified through B6/G6, but **not on `origin/main`** — `app/styles/theme.css` exists only in the identity worktree at `59ea597`. `origin/main` is `fb4d045`. | All visual work. Do not start against `--ve-*`/`--admin-*`; that vocabulary is retired by B2/G2. Branch from a base that carries the cutover. |
...
pass/fail checklist with a reason per condition, prize weights to a share of total draws,
...
**E — Perks.** Perk list and sequenced creation. Prize pool presenting computed odds with
