...
Pre-P1.5F hotfix status: `HOTFIX-P15C-CURRENCY-001` was implemented on 2026-08-18 and is ready for review. It corrects `xp_accounts.accounting_currency` from the legacy `XP` sentinel to a nullable per-account ISO-4217 accounting currency, updates the account-control RPC/overview contract and admin controls, preserves exposure mathematics without FX conversion, regenerates database types and adds database/app regression coverage. It must still be merged before P1.5F implementation begins.
...
P0 CMS remediation, P1 Hybrid LMS foundation and P1.5E are closed. Do not reopen or replace accepted P0/P1/P1.5 architecture unless a Phase 1.5 ticket explicitly requires a narrow extension.

The immediate objective is `HOTFIX-P15C-CURRENCY-001` before beginning P1.5F.
