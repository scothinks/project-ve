...
## Identity adoption foundation, 2026-09-08

B0 / #96 is implemented and locally verified: a production-source token contract
is wired into CI, and 24 declarations for 12 proven-dead tokens are removed.
...
The addendum also governs controlled temporary entitlement grants and organisation AI metering. Temporary higher-plan access, granular temporary privileges, AI trials and AI top-ups must use the central entitlement/grant mechanism defined there, and organisation AI access is governed by the accepted AI metering, budget and abuse-control boundary from P1.5E.

Phase 1.5A through P1.5F are implemented and locally validated as of 2026-08-30.
P0/P1 architecture is closed. Do not begin P2 work until hosted query statistics
...
The first authorised implementation pass was P1.5A. P1.5A received a focused closure pass on 2026-08-08 after acceptance review found gaps in self-service organisation management, organisation CMS authoring/media and learner lesson/quiz context preservation. The final contextual learner browser journey amendment passed on 2026-08-09, so P1.5A is formally closed. Current closure details and validation evidence live in the P1.5 addendum. P1.5B implementation was explicitly requested and kicked off on 2026-08-09 with the mission registry, organisation mission catalogue, programme delivery context, contextual referral and mission outcome context foundation; follow-up P1.5B passes added the first organisation mission create/adapt/edit admin workflow, contextual mission status authorization, programme mission delivery controls, learner-facing contextual invite routing, trusted programme mission award/proof execution for delivery dates and point overrides, and contextual referral qualification tied to scoped programme mission awards. The full local remediation/E2E gate passed on 2026-08-10. Later P1.5B acceptance reviews required focused closure before P1.5C: closure migrations and app changes now add distinct programme mission delivery identities, programme-scoped learner/referral progress, contextual referral programme entry outcomes, org-aware proof review authorization, tenant-bound mission content validation, conservative XP-only reward-mode entitlements, programme-workspace manual referral approval, mission-CMS organisation-wide delivery controls, shared contextual confirmation callback preservation for resend-confirmation, and browser proof of learner proof submission through organisation proof queue approval to learner success state. Final narrow closure validation passed on 2026-08-12 with typecheck, lint, unit tests, build, the 23-file/509-test pgTAP suite and 9/9 Playwright E2E flows. P1.5B is formally closed. P1.5C then received its authorized focused closure review: `P15-XP-001`, `P15-XP-002`, `P15-XP-003`, `P15-XP-004`, `P15-XP-005`, `P15-XP-006` and `P15-RWD-001` were remediated within `P15C-CLOSE-01` through `P15C-CLOSE-05`; a follow-up closure pass resolved exact Org Mode delivery context, contextual lesson completion enforcement and refund-neutral issuance accounting. P1.5D is closed. P1.5E was accepted and closed on 2026-08-15 after focused boundary closure for AI reservation serialization, worker revalidation, active-membership unit supervision and instructor workspace role alignment. `HOTFIX-P15C-CURRENCY-001` and P1.5F are now implemented and included in the final Phase 1 local release gate. P1.5A through P1.5F are locally closed; hosted performance evidence remains the prerequisite for P2.

Where the P1.5 addendum conflicts with earlier assumptions in this product plan, the P1.5 addendum takes precedence.

Planning amendment status: `P15-ENT-002` implements temporary capability grants and entitlement overrides as a P1.5E extension of the `P15-ENT-001` entitlement foundation, and `P15-AI-001` implements the required organisation AI metering, budget and abuse-control enforcement. A focused closure pass added organisation-serialized AI reservations, worker-side role/entitlement/cap/rate/concurrency revalidation and two-session local concurrency coverage. Both tickets are accepted and closed as part of P1.5E. This scope does not create AI wallets, transferable credits, exchanges or marketplaces.

Final Phase 1 closure status, 2026-08-30: `HOTFIX-P15C-CURRENCY-001` and
P1.5F are implemented and locally validated. The closure includes the shared
design/admin system, organisation Overview and People workflows, the Platform
Catalog pseudo-workspace and staffing boundary, organisation reward/XP/
recommendation workflows, and the accepted P0.3/P1.1/P1.2 read-path performance
work. `npm run test:remediation:local` passed with 35 files / 755 pgTAP tests,
all repository/concurrency/economic gates and 21/21 production Playwright tests;
...
**Do not implement until P1.5 is closed and hosted query statistics and plans
...
P0 CMS remediation, P1 Hybrid LMS foundation and the P1.5A–P1.5F implementation
are locally closed. Do not reopen or replace accepted P0/P1/P1.5 architecture
without an explicitly authorised narrow extension.

The next permissible objective is hosted query evidence for the completed
read-path work. Do not begin P2 from local timings alone.
