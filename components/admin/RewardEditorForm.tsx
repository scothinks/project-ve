...
export function RewardEditorForm({
  ...
  const [distributionMode, setDistributionMode] = useState(
    lockDistributionMode ?? reward.distributionMode,
  );
  const [fulfillmentType, setFulfillmentType] = useState(
    reward.fulfillmentType === "perk_bundle"
      ? "manual"
  ...
  const [sharedWithProgrammes, setSharedWithProgrammes] = useState(
    reward.sharedWithProgrammes,
  );
  const [limitPeriod, setLimitPeriod] = useState(reward.limitPeriod);
  const [perUserLimit, setPerUserLimit] = useState(
    Math.max(1, reward.perUserLimit || 1),
  );
  const [claimSteps, setClaimSteps] = useState(
    reward.claimSteps.length > 0
      ? reward.claimSteps
  ...
  const [externalUrl, setExternalUrl] = useState(
    asString(reward.fulfillmentConfig.url),
  );
  ...
  const [nativeAmount, setNativeAmount] = useState(
    asNumber(reward.fulfillmentConfig.amount, 100),
  );
  ...
  const [nativeUses, setNativeUses] = useState(
    asNumber(reward.fulfillmentConfig.uses, 3),
  );
  const initialFallback = (() => {
    const fallback =
      reward.fulfillmentConfig.fallback &&
      typeof reward.fulfillmentConfig.fallback === "object"
  ...
  const [perkFallbackUses, setPerkFallbackUses] = useState(
    asNumber(initialFallback.uses, 3),
  );
  const [partner, setPartner] = useState(
    asString(reward.fulfillmentConfig.partner),
  );
  ...
  return (
    <EconomyForm
      ...
      steps={[
        {
          id: "basics",
          title: "The reward",
          description: "Name the offer and choose its XP cost.",
          content: (
            <>
        ...
        },
        {
          ...
          title: "Distribution",
          description:
            "Choose how learners find and receive this reward. A perk opens a prize pool.",
          content: (
            <>
              <div className="grid gap-4 md:grid-cols-3">
                <label>
                  ...
                  <select
                    className={fieldClasses()}
                    name="campaignId"
                    defaultValue={reward.campaignId ?? ""}
                  >
                ...
                {distributionMode === "direct" ? (
                  ...
                  <div className="rounded-[12px] border border-[var(--ui-border)] bg-[var(--ui-surface-inset)] px-4 py-3">
                    ...
                    <p className="mt-1 text-xs font-semibold leading-5 text-[var(--ui-text-muted)]">
                      Learners redeem this item to reveal a prize. The actual
                      won reward keeps its own fulfillment type.
                    </p>
                    <input
                      name="fulfillmentType"
                      type="hidden"
              ...
              <section className="rounded-[16px] border border-[var(--ui-border-subtle)] bg-[var(--ui-chrome)] p-4">
                ...
                <p className="mt-1 text-xs font-semibold leading-5 text-[var(--ui-text-muted)]">
                  Controls whether programmes can attach this reward for
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-3">
                  <label>
                    ...
                    >
                      ...
                      <option value="organization_owned">
                        Organisation-owned
                      </option>
                      <option value="programme_sponsored">
                        Programme-sponsored
                      </option>
                    </select>
                  </label>
                  ...
                  {ownerScope === "organization_owned" ? (
                    <label>
                      ...
                      <select
                        className={fieldClasses()}
                        name="organizationId"
                        defaultValue={reward.organizationId}
                        required
                      >
                  ...
                  {ownerScope === "programme_sponsored" ? (
                    <label>
                      ...
                      <select
                        className={fieldClasses()}
                        name="sponsoredProgrammeId"
                        defaultValue={reward.sponsoredProgrammeId}
                        required
                      >
                ...
                {ownerScope === "platform_owned" ? (
                  <label className="mt-4 flex items-start gap-3 rounded-[14px] bg-[var(--ui-surface-inset)] p-4 text-sm font-black">
                    <input
                      ...
                      onChange={(event) =>
                        setSharedWithProgrammes(event.target.checked)
                      }
                      type="checkbox"
                    />
                    <span>
                      ...
                      <span className="block text-xs font-semibold leading-5 text-[var(--ui-text-muted)]">
                        Allows selected programmes to attach this Project VE
                        reward.
                      </span>
                    </span>
                  </label>
        ...
        },
        {
          ...
          description:
            "Opening dates control availability. An empty end date leaves the offer open; the provider’s own expiry still applies.",
          content: (
            <>
              <div className="grid gap-4 md:grid-cols-3">
                <label>
                ...
                {limitPeriod === "none" ? (
                  <>
                    <div className="rounded-[12px] border border-[var(--ui-border)] bg-[var(--ui-surface-inset)] px-4 py-3">
                      ...
                      <p className="mt-1 text-xs font-semibold leading-5 text-[var(--ui-text-muted)]">
                        Anyone with enough XP can redeem this reward whenever
                        stock and campaign rules allow.
                      </p>
                    </div>
            ...
            <>
                ...
                fulfillmentType === "manual" ? (
                  <div className="mt-4 space-y-3">
                    {fields.map((field, index) => (
                      <div
                        className="grid gap-2 rounded-[14px] bg-[var(--ui-surface-inset)] p-3 md:grid-cols-[1fr_1fr_8rem_5rem_auto]"
                        key={`field-${index}`}
                      >
                        <input
                          className={fieldClasses()}
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateField(index, {
                              type: event.target.value as RewardField["type"],
                            })
                          }
                          value={field.type}
                        >
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-3">
                    <label>
                      ...
                      <select
                        className={fieldClasses()}
                        onChange={(event) =>
                          setNativeEffect(event.target.value)
                        }
                        value={nativeEffect}
                      >
                    ...
                    {nativeEffect === "xp_bonus" ? (
                      <label>
                        ...
                        <input
                          ...
                          min={1}
                          onChange={(event) =>
                            setNativeAmount(Number(event.target.value))
                          }
                          type="number"
                          value={nativeAmount}
                        />
                      ...
                      <>
                        <label>
                          ...
                          <input
                            ...
                            min={1.01}
                            onChange={(event) =>
                              setNativeMultiplier(Number(event.target.value))
                            }
                            step="0.1"
                            type="number"
                        ...
                        <label>
                          ...
                          <input
                            ...
                            min={1}
                            onChange={(event) =>
                              setNativeDurationHours(Number(event.target.value))
                            }
                            type="number"
                            value={nativeDurationHours}
                          />
                        </label>
                        <label>
                          ...
                          <input
                            ...
                            min={1}
                            onChange={(event) =>
                              setNativeUses(Number(event.target.value))
                            }
                            type="number"
                            value={nativeUses}
                          />
                ...
                {distributionMode === "perk_bundle" ? (
                  <div className="mt-4 space-y-4">
                    <div className="rounded-[14px] bg-[var(--ui-surface-inset)] p-4">
                      ...
                      <p className="mt-1 text-xs font-semibold leading-5 text-[var(--ui-text-muted)]">
                        Used if every configured perk prize is sold out, capped,
                        or not yet released.
                      </p>
                    </div>
                    <div className="grid gap-4 md:grid-cols-3">
                      <label>
                        ...
                        <select
                          className={fieldClasses()}
                          onChange={(event) =>
                            setPerkFallbackType(event.target.value)
                          }
                      ...
                      <label className="md:col-span-2">
                        ...
                        <input
                          className={fieldClasses()}
                          onChange={(event) =>
                            setPerkFallbackTitle(event.target.value)
                          }
                      ...
                      <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <label>
                          ...
                          <input
                            ...
                            min={1}
                            onChange={(event) =>
                              setPerkFallbackUses(Number(event.target.value))
                            }
                            type="number"
                      ...
                      <div className="grid gap-4 md:grid-cols-3">
                        <label>
                          ...
                          <input
                            ...
                            min={1}
                            onChange={(event) =>
                              setPerkFallbackAmount(Number(event.target.value))
                            }
                            type="number"
                ...
                (fulfillmentType === "voucher_code" ||
                  ...
                  <div className="mt-4">
                    ...
                    <p className="mt-2 text-xs font-semibold text-[var(--ui-text-muted)]">
                      Codes and QR payloads are uploaded as batches from the
                      inventory page.
                    </p>
                  </div>
                ) : null}
              ...
              <label className="flex items-center gap-3 rounded-[14px] bg-[var(--ui-surface-inset)] px-4 py-3 text-sm font-black">
                <input
                  name="isEnabled"
                  type="checkbox"
                  defaultChecked={reward.isEnabled}
                />
                Enabled
              </label>
            </>
      ...
      reviewAction={
        <>
          <SubmitButton
            label={mode === "create" ? "Create reward" : "Save reward"}
          />
        </>
      }
    >
      {mode === "edit" ? (
        <input name="rewardId" type="hidden" defaultValue={reward.id} />
      ) : null}
      <input
        name="claimSteps"
        type="hidden"
