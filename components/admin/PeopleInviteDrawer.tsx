...
export function PeopleInviteDrawer({
  defaultOpen = false,
  organizationId,
  targetOptions,
}: {
  ...
  targetOptions: AdminPeopleTargetOption[];
}) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void searchInviteCandidates(organizationId, query)
        .then((candidates) => {
  ...
  }, [method, organizationId, query]);

  return (
    <AdminDrawer
      description="Add new members to the organisation."
    ...
    >
      <form action={sendInvitation} className="flex flex-col gap-6">
