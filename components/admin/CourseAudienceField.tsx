export function CourseAudienceField({ defaultValue }: { defaultValue: string }) {
  return (
    <label className="block">
      ...
      <p className="mt-1 text-xs font-semibold leading-5 text-[var(--admin-on-surface-variant)]">
      ...
      <textarea
        className="mt-3 min-h-24 w-full resize-none rounded-[14px] border border-[var(--admin-border-warm)] bg-[var(--admin-surface)] px-4 py-3 text-sm font-bold text-[var(--admin-on-surface)] outline-none transition focus:border-[var(--admin-primary)]"
