...
export function CourseGenerateAction({ kind, lessons, questions = 0, retryId, enabled, busy, valid = true, label, onGenerate }: {
  ...
  return <div className="space-y-3">
    <p className="text-sm leading-6">{!price ? enabled ? error ? 'The price could not be loaded.' : 'Preparing the price for this scope…' : 'Generation is unavailable. Your saved results remain accessible.'
      : !price.metered ? 'Platform Catalog · No organisation credits used.'
    ...
    {price && <p className="text-xs leading-5">{kind === 'course_outline' ? 'Review and edit the outline before choosing whether to draft the course.' : 'You’ll review the result before saving. Nothing is published automatically.'} Images are a separate, optional choice.</p>}
  </div>;
}
