...
export function AiCoursePreview({ result }: { result: CourseResult }) {
  return <section aria-label="Completed lesson previews" className="space-y-5">{[...(result.candidate?.completed ?? [])].sort((a,b)=>a.index-b.index).map(({index,lesson},position)=><details key={index} className="rounded-2xl border border-[var(--ui-border-subtle)] p-4" open={position===0}>
    ...
    {lesson.questions.map((q,i)=><article key={i} className="mt-4 rounded-xl bg-[var(--ui-surface-soft)] p-4 text-sm"><h4 className="font-bold">{q.prompt}</h4><ul className="my-3 space-y-2">{q.options.map((o,n)=><li key={n}>{o.label}{o.isCorrect&&<strong> · Correct answer</strong>}</li>)}</ul><p>{q.explanation}</p></article>)}
  </details>)}</section>;
}
