...
export function AiCoursePreview({ result }: { result: CourseResult }) {
  return <section aria-label="Completed lesson previews" className="space-y-5">{[...(result.candidate?.completed ?? [])].sort((a,b)=>a.index-b.index).map(({index,lesson})=><details key={index} className="rounded-2xl border border-[var(--admin-border-warm)] p-4" open>
    <summary className="cursor-pointer font-bold">Lesson {index+1}: {lesson.title}</summary><p className="my-3 text-sm">{lesson.description}</p>
