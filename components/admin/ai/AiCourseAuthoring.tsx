  ...
  return <div data-outline-revision={result?.outlineRevision} className="mx-auto w-full max-w-4xl space-y-6 px-1 pb-28 pt-4 md:py-8">
    ...
    {availability.enabled && <div hidden={!!result || !!id}>
      <AiCourseDiscovery brief={brief} onChange={journey.setBrief} disabled={busy} guidanceEnabled={availability.guidanceEnabled} initialMode={initialId ? 'direct' : 'start'}>
        <CourseGenerateAction kind="course_outline" lessons={brief.lessonCount} enabled={!id && enabled} busy={busy} valid={!!brief.need.trim() && !!brief.audience.trim()} label="Generate" onGenerate={price => void run(() => journey.generateOutline(price))} />
      </AiCourseDiscovery>
    </div>}
    ...
    {result?.deleted ? <p>This result was deleted. Any saved course remains in Courses.</p> : result && <>
      ...
      {(active || result.stage === 'stopped') && <AdminCard><div ref={progressRef} tabIndex={-1} role="status" aria-live="polite"><strong>{result.stage === 'stopped' ? 'Generation stopped' : result.stopRequested ? 'Stopping…' : result.kind === 'course_outline' ? 'Planning your course' : `${result.completedCount} of ${result.totalCount} lessons ready`}</strong><p className="mt-2 text-sm leading-6">{result.stage === 'stopped' ? 'Completed work remains available below. You can leave and resume it later.' : delayed ? 'This is taking longer than expected. Your saved checkpoints remain available.' : result.stage === 'starting' ? 'Your request was accepted. Waiting for the writer to start.' : 'Each finished lesson appears after it has been checked and saved.'}</p></div></AdminCard>}
