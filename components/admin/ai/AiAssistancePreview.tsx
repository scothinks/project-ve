...
export function AiAssistancePreview({ result, selection, setSelection, disabled, onDraft }: {
  ...
  return <section className="space-y-5" aria-label="Generated result">
    ...
    {'questions' in candidate && candidate.questions.map((q, i) => <article key={i} className="space-y-3 rounded-2xl border border-[var(--admin-border-warm)] p-5">
    ...
    {'suggestions' in candidate && candidate.suggestions.map((s, i) => <article key={i} className="space-y-3 rounded-2xl border border-[var(--admin-border-warm)] p-5">
