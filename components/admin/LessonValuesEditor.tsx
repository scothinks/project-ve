...
export function LessonValuesEditor({ lessonId, dimensions, tags }: {
  ...
  return <div className="space-y-8">
    <fieldset className="space-y-4" disabled={saving}>
      <legend className="mb-4 text-sm font-extrabold">Choose values <span className="ml-2 font-medium text-[var(--ui-text-muted)]">{choices.length} selected</span></legend>
      {available.length > 6 ? <input aria-label="Find a value" className="w-full rounded-xl border border-[var(--ui-control-border)] bg-[var(--ui-surface)] px-4 py-3 text-sm outline-none focus:border-[var(--ui-focus)]" onChange={(event) => setSearch(event.target.value)} placeholder="Find a value…" type="search" value={search} /> : null}
      <div className="grid gap-3 sm:grid-cols-2">
        {visible.map((dimension) => {
          const selected = choices.some((choice) => choice.dimensionId === dimension.id);
          return <label className={cn("flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition-colors focus-within:ring-2 focus-within:ring-[var(--ui-focus)]", selected ? "border-[var(--ui-action)] bg-[var(--ui-surface-soft)]" : "border-[var(--ui-border-subtle)] bg-[var(--ui-surface)] hover:border-[var(--ui-action)]")} key={dimension.id}>
            <input aria-label={dimension.label} checked={selected} className="mt-1 h-4 w-4 shrink-0 accent-[var(--ui-action)]" onChange={() => toggleValue(dimension.id)} type="checkbox" />
            <span><span className="block text-sm font-extrabold">{dimension.label}</span><span className="mt-1 block text-xs leading-5 text-[var(--ui-text-muted)]">{dimension.description}</span></span>
          </label>;
        })}
      </div>
      {!visible.length ? <p className="py-3 text-sm text-[var(--ui-text-muted)]">{available.length ? "No matching values. Try another search." : "No values are available yet. You can continue to Review."}</p> : null}
    </fieldset>

    {choices.length ? <div className="space-y-4">
      <div><h2 className="text-lg font-extrabold">Set the focus</h2><p className="mt-1 text-sm text-[var(--ui-text-muted)]">How much attention does each value get?</p></div>
      {choices.map((choice) => {
        ...
        return <section aria-label={`${label} focus`} className="rounded-2xl border border-[var(--ui-border-subtle)] bg-[var(--ui-surface)] p-5 sm:p-6" key={choice.dimensionId}>
          <div className="mb-4 flex items-center justify-between gap-3"><h3 className="font-extrabold">{label}</h3><button aria-label={`Remove ${label}`} className="rounded-lg px-2 py-1 text-xs font-bold text-[var(--ui-text-muted)] hover:bg-[var(--ui-surface-soft)]" disabled={saving} onClick={() => toggleValue(choice.dimensionId)} type="button">Remove</button></div>
          <fieldset className="grid gap-2 sm:grid-cols-3" disabled={saving}>
            <legend className="sr-only">Focus for {label}</legend>
            {options.map((option) => <label className={cn("flex cursor-pointer items-start gap-2 rounded-xl border px-3 py-3 focus-within:ring-2 focus-within:ring-[var(--ui-focus)]", choice.weight === option.value ? "border-[var(--ui-action)] bg-[var(--ui-surface-soft)]" : "border-[var(--ui-border-subtle)]")} key={option.value}>
              <input checked={choice.weight === option.value} className="mt-0.5 accent-[var(--ui-action)]" name={`focus-${choice.dimensionId}`} onChange={() => updateChoice(choice.dimensionId, { weight: option.value })} type="radio" />
              <span><span className="block text-sm font-bold">{option.label}</span><span className="mt-1 block text-xs leading-4 text-[var(--ui-text-muted)]">{option.detail}</span></span>
            </label>)}
          </fieldset>
          <details className="mt-4 border-t border-[var(--ui-border-subtle)] pt-4 text-sm">
            <summary className="cursor-pointer font-bold text-[var(--ui-text-muted)]">Learner guidance <span className="font-normal">· Optional</span></summary>
    ...
    </div> : <p className="text-sm text-[var(--ui-text-muted)]">You can leave this blank and come back later.</p>}

    <div className="sticky bottom-0 z-20 -mx-4 border-t border-[var(--ui-border-subtle)] bg-[var(--ui-surface)] px-4 py-4">
      {error ? <p className="mb-3 text-sm font-semibold text-[var(--ui-danger)]" role="alert">{error}</p> : null}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button className="rounded-full px-3 py-3 text-sm font-bold text-[var(--ui-text-muted)] disabled:opacity-60" disabled={saving} onClick={() => { void continueTo(`${base}/quiz`); }} type="button">Back to quiz</button>
        <button className="rounded-full bg-[var(--ui-action)] px-5 py-3 text-sm font-extrabold text-[var(--ui-on-action)] disabled:opacity-60" disabled={saving} onClick={() => { void continueTo(`${base}/preview`); }} type="button">{saving ? "Saving…" : "Continue to Review"}</button>
      </div>
    </div>
  </div>;
}
