...
export function MediaManager() {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap gap-2" aria-label="Media filter">{[["organization", "All workspace media"], ["generated", "Generated"], ["unused", "Unused"]].map(([value, label]) => <button key={value} className={`${button} aria-pressed:bg-[var(--ui-success-bg)]`} type="button" aria-pressed={source === value} onClick={() => { setSource(value); setOffset(0); }}>{label}</button>)}</div>
